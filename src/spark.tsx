  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-surface">
      <div className="flex items-center gap-2 border-b border-border-subtle bg-surface-warm/40 px-4 py-2">
        <span className="rounded-full bg-accent-soft px-2 py-0.5 text-[11px] text-accent">{s.category}</span>
        {s.ts > 0 && <span className="font-mono text-[11px] text-text-muted" title={new Date(s.ts).toLocaleString()}>{fmtStamp(s.ts)}</span>}
        <span className="ml-auto flex items-center gap-1.5 text-[11px] text-text-muted" title={`Generated by ${s.modelLabel} at ${new Date(s.ts || Date.now()).toLocaleString()}`}>
  ...
  return (
    <div className="w-full">
      <SettingsHeader
        icon={Dices}
        title="Spark"
        subtitle="A nudge out of the bubble."
      />
